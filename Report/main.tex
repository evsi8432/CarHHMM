...
Data sets comprised of sequences of curves sampled at high-frequency in time are increasingly common in practice, but they can exhibit complicated dependence structures which cannot be modelled using common methods of Functional Data Analysis (FDA). We detail a hierarchical approach which treats the curves as observations from a hidden Markov model (HMM). The distribution of each curve is then defined by another fine-scale model which may involve a variety of methods such as moving-window transformations, Fourier analysis, and auto-regression. This approach is broadly applicable to sequences of curves exhibiting intricate dependence structures. As a case study, we use this framework to model the movement of a killer whale off the western coast of Canada. Through simulations we show that our model produces more interpretable state estimation and more accurate parameter estimates compared to existing baselines.
