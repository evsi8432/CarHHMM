...
All components of $\Zone_{t,\tilde t^*}$ are assumed to be Normally distributed as in Equation (\ref{eqn:carhmm}) and independent of one another when conditioned on the dive types and subdive states. Each component is assumed to have its own mean and variance parameters, but all components share the same auto-correlation parameter. 
