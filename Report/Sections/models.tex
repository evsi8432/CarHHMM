...
Hidden Markov models can describe state-switching Markovian processes in discrete time and will be the core structure of the models we use to describe $Y$ and $Y^*$. %Traditional HMMs model process auto-correlation by assuming that the underlying behavior of a processes follows a single Markov chain. However, when conditioned on the hidden state sequence observations are assumed to be independent. Therefore, classical HMMs do not hold when the observations exhibit other forms of significant correlation in time. Examples of this include continuous-time processes such as a Wiener process, systems which exhibit temporal dependencies acting at several different time scales \citep{Barajas:2017}, and periodic behaviour including the motion of an ideal spring.  
...
Figure \ref{fig:models} represents the dependence structure for an HHMM. Due to the nested structure of the hierarchical hidden Markov model, the likelihood is easy to calculate via the forward algorithm.
%
...
For our case study, we use the discrete Fourier transform (DFT) of a moving window of width $h$ across $Y^*$:
%
\begin{align*}
