...
%Hidden Markov models describe state-switching Markovian processes in discrete time and is the core structure we use to model $Y$ and $Y^*$. We first detail the structure of a traditional HMM, followed by the conditionally auto-regressive hidden Markov model (CarHMM) \citep{Lawler:2019}, a variation which explicitly models auto-correlation between observations. 
%Either the HMM or the CarHMM can model either the coarse scale sequence $Y$ or any of the fine scale sequences, $Y_1^*,\ldots, Y_T^*$. 
%We then describe a hierarchical hidden Markov model (HHMM) \citep{Barajas:2017,Adam:2019} to simultaneously model $Y$ and $Y^*$. 
%, which is comprised of two distinct, but related, HMMs that jointly model these time series. 
%Next, we outline a method to reduce fine-scale correlation by transforming raw observations into meaningful summary statistics. In particular, we apply a discrete Fourier transform (DFT) to the components of $Y^*$ using a moving window to capture its periodic behaviour. Finally, we show how each of these models can be altered and combined to form a wide variety of new models.
...
%
%Once $\Z_t$ is calculated, it simply replaces the original fine-scale observations $Y^*_t$ in the fine-scale HMM structure. As such, the emission distributions and the 
