...
\[
    \{Y^*_{t,100(\tilde t^* - 1) + 1},\ldots,Y^*_{t,100\tilde t^*}\} = IDFT\left\{\hat{Y}^{*(0)}_{t,\tilde t^*},
...
%
where $P(\z_{t,\tilde t^*}\mid\z_{t,\tilde t^*-1};\theta^*)$ is an $N^* \times N^*$ diagonal matrix with $(i^*,i^*)^{th}$ entry equal to $f^{*(\cdot,i^*)}(\z_{t,\tilde t^*}\mid\z_{t,\tilde t^*-1}; \theta^{*(\cdot,i^*)})$.
%
