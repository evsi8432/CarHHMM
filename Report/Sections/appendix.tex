...
Assume that $X$, $Y$, and $X^*$ are known and simulated using the procedure detailed in Section \ref{sec:sim_study}, leaving only $Y^*$ to be simulated. Let $DFT\{Y^*_{t,t^*}\}(k)$ be denoted as $\hat{Y}^{*(k)}_{t,t^*}$ and let $DFT\{Y^*_{t,t^*}\}$ be denoted as $\hat{Y}^*_{t,t^*}$ for convenience. We simulate $\hat Y^*$ and then take the inverse $DFT$ to find $Y^*$. 
...
\begin{enumerate}
    ...
    \item Next, we simulate $\hat{Y}^{*(0)}_{t,101},\enspace \hat{Y}^{*(0)}_{t,201}, \enspace \ldots$ using Equation (\ref{eqn:yhat_0}):
    \begin{equation}	
    ...
    $$
    where $a$ is equals 1 or -1 with probability 1/2 and $b^{(k)}_{t,t^*}$ has the following distribution:
    \begin{align*}
    	...
    	\end{array}\right. 
    \end{align*}
    This means that the expected values of Fourier modes 1 and 2 are much higher for subdive state 2. Note that $\mathbb{E}\left(|\hat{Y}^{(k)}_{t,t^*}|^2\right)$ is proportional to $1/k^2$. This silences the higher Fourier modes, which ``smooths out" the raw acceleration. 
    \item For Fourier modes 50 through 99, we use the following procedure to ensure that the inverse DFT is real-valued:
    $$
...
We now prove that this construction of $Y^*$ results in the correct distributions listed in Section \ref{sec:sim_study}. To find the distribution of $\Zone_{t,t^*}$, use Equation (\ref{eqn:yhat_0}) and note that $\Zone_{t,t^*} = \text{Re}\left(\hat{Y}^{(0)}_{t,t^*}\right) = \hat{Y}^{(0)}_{t,t^*}$, since the 0th Fourier mode is equal to the average value of an interval and $\hat{Y}^{(0)}_{t,t^*}$ is real-valued by construction:
%
