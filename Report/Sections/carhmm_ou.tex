...
Although most continuous time models are difficult to incorporate into an HHMM, we show in theorem (1) that under certain conditions, the CarHMM is equivalent to a state-switching Ornstein-Uhlenbeck process shown in equation (\ref{eqn:OU_sol}). This gives new interpretation to the learned parameters of the CarHMM in the context of a continuous-time model.
...
\end{enumerate}
then the observations $y$ are equivalent to the output of a conditionally auto-regressive hidden Markov model with normal emission distributions and parameters $\Theta = (\theta^{(1)}, \ldots, \theta^{(N)}); \enspace \theta^{(i)} = \{\mu^{(i)},\sigma^{2(i)},\phi^{(i)}\}$, where:

\begin{align}
\mu^{(i)} = \gamma^{(i)}, \qquad \sigma^{2(i)} = \frac{\omega^{(i)}^2}{2\beta^{(i)}} (1-e^{-2\beta^{(i)}\Delta t}), \qquad \phi^{(i)} = e^{-\beta^{(i)}\Delta t} \label{eqn:CarHMM_to_OU}
\end{align}
